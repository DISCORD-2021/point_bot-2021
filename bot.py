...
async def on_message(message):
    ...
    if message.content.startswith(f'{command_prefix}도움'):
        ...
        embed.add_field(name="**내정보**", value="**`각자의 정보를 보여줍니다.`**", inline=False)
    ...
    if message.content.startswith(f'{command_prefix}관리'):
        ...
        #embed.add_field(name="**언뮤트**", value="**`언뮤트 (userid) 를 입력할 경우, (userid)는 언뮤트 됩니다.`**", inline=False)
        embed.add_field(name="**내정보**", value="**`각자의 정보를 보여줍니다.`**", inline=False)
    ...
    #뮤트해제 명령어 구문
    if(message.content.split(" ")[0] == f'{command_prefix}언뮤트'):
        if(message.author.guild_permissions.manage_channels):
            try:
                user = message.guild.get_member(
                    int(message.content.split(' ')[1][3:21]))
                ...
                await message.channel.send(embed=discord.Embed(title="🛑 (언뮤트)처리 도중 에러 발생", description=str(e), color=0xff0000))
                return
        else:
    ...
    #정보 명령어 구문
    if message.content.startswith(f'{command_prefix}내정보'):
        user = message.author
    ...
    if message.content.startswith(f'{command_prefix}인증'):
        ...
        if user == 677821350114492446:
            ...
            await asyncio.sleep(1)
            await message.channel.send((f"{message.author.mention}") + '님은 인증되지 않은 유저 입니다 ❗ ' + '인증채널에서 p인증 을 입력해서 인증해주세요! 💨💨')

    if message.content.startswith(f'{command_prefix}테스트'):
        file = open("./유저/유저.json", "w")
        while True:
            file.write(f"{message.author.id}")
            await message.channel.send((f"{message.author.mention}") + '님의 가입되었습니다 ❗ ' + '봇을 이제 재미있게 사용해주세요~! 💨💨')
            file.close()

    #(아이템) 상점 명령어 구문
    if message.content.startswith(f'{command_prefix}상점'):
        embed = discord.Embed(title="아이템 상점 🔖", description="🛒 아이템 목록! 다같이 쇼핑하자!!", color=0x00aaaa)
        embed.add_field(name="💴 등급 UP! 조각", value="등급업 아이템을 합성 하기 위한 재료 - 5BP 😆", inline=False)
        embed.add_field(name="🔖 경고제거권(1회)", value="자신의 경고를 제거해주는 마법의 아이템! - 30BP 😙", inline=False)
        embed.add_field(name="📜 칭호 랜덤 뽑기권(1회)", value="등급업을 하기 위한 일회용 아이템! - 5BP 😮", inline=False)
        embed.add_field(name="🧾 랜덤 아이템 뽑기권(1회)", value="등급업을 하기 위한 일회용 아이템! - 5BP 😛", inline=False)
